...
const ByPerDayHourOfWeekDay = require('./byPerDayHourOfWeekDay');
...
function createInstance(gitDir, type, options) {
  switch (type) {
    case statisticsTypes.BY_PER_DAY_HOUR_OF_WEEK_DAY:
      return new ByPerDayHourOfWeekDay(gitDir, options);
